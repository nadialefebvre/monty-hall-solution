...
class MontyHallSimulationTest extends AnyWordSpec with Matchers {
  ...
  "The Monty Hall game" when {
    ...
    "handling arguments input scenarios" should {

      /** Tests the behavior of the program when handling a valid argument. */
      "parse the number of trials from command line arguments" in {
        val args = Array("1000")
        MontyHallSimulation.parseNumTrials(args) shouldBe 1000
      }

      /** Tests the behavior of the program when handling invalid arguments. */
      "use default numTrials when invalid arguments are provided" in {
        val defaultNumTrials = MontyHallSimulation.DefaultNumTrials
        val invalidArgs = Seq(
          Array.empty[String],
          Array("invalid"),
          Array("invalid", "arguments"),
          Array("-1"),
          Array("0"),
          Array("1,5"),
          Array("1.5")
        ...
        invalidArgs.foreach { args =>
          MontyHallSimulation.parseNumTrials(args) shouldBe defaultNumTrials
        }
      }
    }
  }
